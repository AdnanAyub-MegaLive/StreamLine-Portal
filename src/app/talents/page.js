...
export default async function TalentsPage() {
  ...
  const deviceData = devices.map((device) => ({ userId:device.talent.publicId,userName:device.talent.displayName,ip:device.lastLoginIp ?? "—",mac:device.macAddress,location:device.location ?? "Unknown",isUserBanned:device.talent.status==="BANNED",isDeviceBanned:device.isBanned }));
  const talentModules={
