        ...
        <nav className="mt-12 space-y-1">
          {nav.map(([label, href]) => (
            <Link
              key={label}
              href={href}
              className={`block rounded-xl px-4 py-3 text-sm ${label === "Host Management" ? "bg-white/10 font-semibold text-[#62e0d0]" : "text-[#a9c5c1] hover:bg-white/5 hover:text-white"}`}
            >
              {label}
            </Link>
          ))}
