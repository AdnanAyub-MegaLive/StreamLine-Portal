...
export default async function AuditLogsPage() {
  ...
  const logs=records.map((log)=>({id:log.id,createdAt:log.createdAt.toLocaleString("en-US"),adminName:log.admin?.name??"System",adminEmail:log.admin?.email??"Automated process",category:log.category,action:log.action,entityId:log.entityId,description:log.description.replace(/\btalents\b/gi,"hosts").replace(/\btalent\b/gi,"host"),reason:log.metadata&&typeof log.metadata==="object"&&!Array.isArray(log.metadata)?log.metadata.reason??null:null}));
