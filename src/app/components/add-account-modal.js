...
export default function AddAccountModal({ type }) {
  ...
  return <>
    <button onClick={() => setOpen(true)} className="flex h-10 items-center justify-center gap-2 rounded-lg bg-[#087f74] px-4 text-xs font-bold text-white shadow-[0_8px_20px_rgba(8,127,116,.2)] transition hover:bg-[#076f66]"><span className="text-lg leading-none">+</span>{isTalent ? "Add host" : "Add user"}</button>
    {open && <div className="fixed inset-0 z-50 grid place-items-center overflow-y-auto bg-[#061c1a]/60 p-4 backdrop-blur-[2px]" role="dialog" aria-modal="true" aria-labelledby="add-account-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <div className="my-auto w-full max-w-[560px] rounded-2xl bg-white shadow-[0_24px_80px_rgba(0,0,0,.25)]">
        <div className="flex items-start justify-between border-b border-[#e5ecea] px-6 py-5"><div><span className="mb-3 grid h-9 w-9 place-items-center rounded-full bg-[#e5f5f2] text-lg font-bold text-[#087f74]">+</span><h2 id="add-account-title" className="text-lg font-bold text-[#172f2b]">{isTalent ? "Add new host" : "Add new user"}</h2><p className="mt-1 text-xs text-[#748782]">{isTalent ? "Create a streamer or audio-room host profile." : "Create a new sender account on the platform."}</p></div><button onClick={() => setOpen(false)} className="grid h-8 w-8 place-items-center rounded-lg text-xl text-[#7c8f8a] hover:bg-[#f0f5f4]" type="button" aria-label="Close modal">×</button></div>
        <form onSubmit={submit} className="p-6">
          <div className="grid gap-5 sm:grid-cols-2">
            ...
            {isTalent && <Field label="Display name" id="talent-display-name"><input className={inputClass} id="talent-display-name" name="displayName" required placeholder="Public host name" /></Field>}
            ...
            {!isTalent && <Field label="Password" id="user-password"><input className={inputClass} id="user-password" name="password" type="password" required minLength="8" placeholder="Minimum 8 characters" /></Field>}
            ...
            {isTalent ? <><Field label="Host type" id="talent-type"><select className={inputClass} id="talent-type" name="talentType" required defaultValue=""><option value="" disabled>Select host type</option><option>Video Streamer</option><option>Audio Room Host</option><option>Video & Audio Host</option></select></Field><Field label="Verification status" id="verification-status"><select className={inputClass} id="verification-status" name="verificationStatus" defaultValue="Pending"><option>Pending</option><option>Under Review</option><option>Verified</option></select></Field></> : <><Field label="Account status" id="user-status"><select className={inputClass} id="user-status" name="status" defaultValue="Active"><option>Active</option><option>Pending</option><option>Suspended</option></select></Field><Field label="User type" id="user-type"><select className={inputClass} id="user-type" name="userType" defaultValue="Standard"><option>Standard</option><option>VIP User</option></select></Field></>}
          </div>
          ...
          <div className="mt-6 flex justify-end gap-2 border-t border-[#e8efed] pt-5"><button type="button" onClick={() => setOpen(false)} className="h-10 rounded-lg border border-[#d7e3e0] px-4 text-xs font-bold text-[#5d716c] hover:bg-[#f5f8f7]">Cancel</button><button type="submit" disabled={pending} className="h-10 rounded-lg bg-[#087f74] px-5 text-xs font-bold text-white shadow-[0_6px_16px_rgba(8,127,116,.2)] hover:bg-[#076f66] disabled:cursor-wait disabled:opacity-60">{pending ? "Saving..." : isTalent ? "Add host" : "Add user"}</button></div>
        </form>
      </div>
    </div>}
  </>;
