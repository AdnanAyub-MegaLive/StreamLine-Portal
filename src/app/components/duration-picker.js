...
export default function DurationPicker({name="durationMinutes",label="Time period",defaultMinutes=10080}){
  ...
  return <div>
    ...
    <select value={selection} onChange={(event)=>setSelection(event.target.value)} className="h-11 w-full rounded-lg border border-[#dce6e4] bg-white px-3 text-xs outline-none focus:border-[#2ca89c]"><option value="4320">3 days</option><option value="10080">7 days</option><option value="21600">15 days</option><option value="43200">30 days</option><option value="custom">Custom time period</option></select>
