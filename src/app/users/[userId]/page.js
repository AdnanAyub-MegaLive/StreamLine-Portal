...
export default async function UserProfilePage({params}) {
  ...
  const user=await prisma.user.findFirst({where:{publicId:decodeURIComponent(userId),deletedAt:null},include:{devices:{orderBy:{lastLoginAt:"desc"},take:1},uploadAssignments:{where:{OR:[{expiresAt:null},{expiresAt:{gt:new Date()}}]},select:{assignedAt:true,expiresAt:true,asset:{select:{publicId:true,name:true,category:true,fileName:true,mimeType:true,fileSize:true,isRoomBackground:true}}},orderBy:{assignedAt:"desc"}},_count:{select:{sentGifts:true}}}});
  if(!user) notFound();
  const device=user.devices[0];
  const profile={id:user.publicId,name:user.name,email:user.email,phone:user.phone,country:user.country ?? "—",gender:user.gender?display(user.gender):"Not set",dob:user.dob?.toISOString().slice(0,10)??"Not set",role:display(user.role),status:display(user.status),vipLevel:user.vipLevel,joined:user.createdAt.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"}),totalSpent:Number(user.totalSpent),balance:Number(user.coinBalance),gifts:user._count.sentGifts,lastLogin:device?.lastLoginAt?.toLocaleString("en-US") ?? "Never",ip:device?.lastLoginIp ?? "—",mac:device?.macAddress ?? "—",location:device?.location ?? "Unknown",assignedAssets:user.uploadAssignments.map(({asset,assignedAt,expiresAt})=>({id:asset.publicId,name:asset.name,category:display(asset.category),fileName:asset.fileName,mimeType:asset.mimeType,fileSize:asset.fileSize,isRoomBackground:asset.isRoomBackground,assignedAt:assignedAt.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"}),expiresAt:expiresAt?.toLocaleString("en-US")??"Never",url:`/api/uploads/${asset.publicId}/file`}))};
