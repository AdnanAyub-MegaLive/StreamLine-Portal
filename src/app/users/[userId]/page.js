...
export default async function UserProfilePage({params}) {
  ...
  const user=await prisma.user.findUnique({where:{publicId:decodeURIComponent(userId)},include:{devices:{orderBy:{lastLoginAt:"desc"},take:1},_count:{select:{sentGifts:true}}}});
  if(!user) notFound();
