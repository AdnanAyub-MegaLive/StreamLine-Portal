...
export default async function UsersPage() {
  ...
  const userData = users.map((user) => ({ id:user.publicId,name:user.name,email:user.phone,phone:user.phone,actualEmail:user.email,initials:user.name.split(" ").map((part)=>part[0]).join(""),role:display(user.role),status:display(user.status),joined:user.createdAt.toLocaleDateString("en-US",{month:"short",day:"2-digit",year:"numeric"}),streams:user._count.sentGifts,vipLevel:user.vipLevel,vip:user.vipLevel > 0,totalSpent:Number(user.totalSpent),balance:Number(user.coinBalance),gifts:user._count.sentGifts }));
