...
export default async function UsersPage() {
  ...
  const userModules={
    ...
    specialIds:users.flatMap((user)=>user.specialIds.map((item)=>({id:item.id,userId:user.publicId,user:user.name,specialId:item.specialId,category:item.definition?.category??"LEGACY",source:item.source,status:item.status,starts:item.startsAt.toLocaleString("en-US"),expires:item.expiresAt?.toLocaleString("en-US")??"Legacy"}))),
